<template>
  <contentful-page-link :class="['u-article-teaser-card', 'cut-off-shape']" :url="url">
    <contentful-image
      class="u-article-teaser-card__image-wrapper"
      v-bind="image"
      :width="480"
      :height="270"
      lazy
    ...
    <div class="u-article-teaser-card__category" v-if="category">{{ category.label }}</div>

    <base-headline tag="h4" class="u-article-teaser-card__headline">{{ headline }}</base-headline>

    <div class="u-article-teaser-card__date">{{ publishDate }}</div>
