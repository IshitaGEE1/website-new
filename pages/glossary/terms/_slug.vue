<template>
  <base-page class="u-glossary-page" :content="content" :footer="footer" :header="header">
    ...
    <section class="u-content-type u-content-type--module u-event">
      ...
      <element-glossary-category-filter
...
export default {
  name: 'GlossaryListingGroup',
  components: { BasePage },
  mixins: [FetchPageDataMixin],
  async asyncData(ctx) {
    return {
      ...await fetchPageData(ctx, ctx.i18n.locale, ContentTypes.pages.glossaryListingGroup),
      glossary: await gqlClient.getGlossaryPagesByRange(ctx.i18n.locale, ctx.route.params.slug)
    }
  },
  computed: {
    headline() {
