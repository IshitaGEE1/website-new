  ...
  computed: {
    _channels() {
      return this.channels.map(c => {
        return {
          name: c.name,
          slug: c.slug && c.channel.url ? [c.channel.url.split('/').reverse()[0], c.slug].join('/') : undefined
        }
      })
    }
  }
...
<template>
  <base-page :content="content" :footer="footer" :header="header" class="u-channels-page">
    <template #pre-content>
      ...
      <content-module>
        <h1 class="u-channels-page__title">Supported channel exports</h1>

        <markdown-text class="u-channels-page__text" :text="fields.text" v-if="fields.text" />

        <channel-grid class="u-channels-page__grid" :channels="_channels" disable-category-filter/>
      </content-module>
    </template>
  </base-page>
</template>
