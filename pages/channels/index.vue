...
import BaseSearch from '../../components/_base/BaseSearch.vue';
...
export default {
  ...
  async asyncData(ctx) {
    return await Promise.all([
      ...
      gqlClient.getSupportedChannels(true, ctx.i18n.locale),
...
<template>
  <base-page :content="content" :footer="footer" :header="header" class="u-channels-page">
    <template #pre-content>
      ...
      <content-module>
        <h1 class="u-channels-page__title">Top channels</h1>

        <markdown-text class="u-channels-page__text" :text="fields.text" v-if="fields.text" />

        <channel-grid :categories="categories" :channels="channels" class="u-channels-page__grid" columns="3"/>
      </content-module>
    </template>
  </base-page>
</template>
